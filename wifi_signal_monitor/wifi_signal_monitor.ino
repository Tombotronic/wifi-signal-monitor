// WiFi signal monitor for M5Stack Cardputer Adv
...
const size_t MAX_LOG_BYTES = 2UL * 1024 * 1024;
...
// header comment) but it stops a stray/hostile device on the LAN from
// blindly POSTing /forget over and over to DoS it into a reboot loop:
// an attacker would first have to load the dashboard to learn the token.
char csrfToken[9];
...
// Tracks live connection state so a dropped WiFi link shows as "OFFLINE"
// instead of silently freezing on the last known-good RSSI reading.
...
void generateCsrfToken() {
  ...
  uint32_t r = esp_random(); // hardware RNG, 32 bits -> 8 hex chars
  for (int i = 0; i < 8; i++) csrfToken[i] = hex[(r >> (i * 4)) & 0xF];
  csrfToken[8] = '\0';
}
  ...
  while (true) {
    ...
    if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed()) {
      ...
      if (status.del && value.length() > 0) {
        ...
        // otherwise backspacing a multi-byte char (e.g. an accented
        // letter in an SSID) leaves a truncated, invalid byte sequence.
...
void rotateLog() {
  ...
  for (size_t i = 0; in.available(); i++) {
    String line = in.readStringUntil('\n');
    if (i >= dropCount && line.length() > 0) out.println(line);
  }
  in.close();
  out.close();
...
// "I" key -> show IP overlay for 5s, then revert to the status screen.
bool showingIp = false;
unsigned long ipShownAtMs = 0;
const unsigned long IP_SHOW_MS = 5000;
...
void drawIp() {
  ...
  M5.Display.setTextSize(UNIT_SCALE);
  M5.Display.setTextColor(M5.Display.color565(150, 150, 150), TFT_BLACK);
  M5.Display.drawString("IP ADDRESS", cx, cy - 30);
...
void drawStatus() {
  ...
  if (wifiConnected) {
    ...
    drawValue(cx, cy1, "---", "OFFLINE", M5.Display.color565(150, 150, 150));
  }
...
void loop() {
  ...
  // Catch connect/disconnect transitions immediately rather than waiting
  // for the next once-a-minute log tick, so OFFLINE shows up right away.
  ...
  if (showingIp) {
    if (millis() - ipShownAtMs >= IP_SHOW_MS) {
      showingIp = false;
      drawStatus();
    }
  } else if (loggedNow) {
